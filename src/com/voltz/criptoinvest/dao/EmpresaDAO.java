...
public class EmpresaDAO {
    ...
    public void cadastrarEmpresa(Empresa empresa) throws SQLException {
        PreparedStatement stmt = conexao.prepareStatement("INSERT INTO T_EMPRESA (cnpj, razao_social) VALUES (?, ?)");
    ...
    public Empresa pesquisarEmpresa(Long id) throws  SQLException, EntidadeNaoEncontradaException {
        PreparedStatement stm = conexao.prepareStatement("SELECT * FROM T_EMPRESA WHERE id_empresa = ?");
        stm.setLong(1, id);
    ...
    public List<Empresa> listarEmpresas() throws SQLException {
        PreparedStatement stm = conexao.prepareStatement("SELECT * FROM T_EMPRESA");
        ResultSet result = stm.executeQuery();
    ...
    public void atualizarEmpresa(Empresa empresa) throws  SQLException {
        PreparedStatement stm = conexao.prepareStatement("UPDATE T_EMPRESA SET cnpj = ?, razao_social = ? WHERE id_empresa = ?");
    ...
    public void removerEmpresa(Long id) throws  SQLException, EntidadeNaoEncontradaException {
        PreparedStatement stm = conexao.prepareStatement("DELETE FROM T_EMPRESA WHERE id_empresa = ?");
        stm.setLong(1, id);
